...
from crec.layers.activation import get_activation
...
class CRECheads(nn.Module):
    def __init__(
        ...
        """
        Contain two prediction heads to predict: bbox and counterfactual cls label
        """
        super().__init__()
        ...
        self.cf_classifier  = nn.Sequential(
            nn.Flatten(),
            nn.Linear(1024,64),
            nn.ReLU(),
            nn.Linear(64,2),
    ...
    def forward(self, xin, cin, yin=None, labels=None, imgs=None,):
        outputs = []
        origin_preds = []
        x_shifts = []
        y_shifts = []
        ...
        if labels is not None:
            cls_id = labels[:,:,4].reshape(-1).to(torch.int64)
            labels = labels[:,:,:4]
            labels*=xin.size()[-1]*self.strides[0]
        
        # predict cls_label for counterfactual classification
        reg_x = self.stems[0](cin)
        reg_feat = self.reg_convs[0](reg_x)
        ks = math.floor(reg_feat.shape[2]/2.0)     # reg_feat.shape[2]: 7~19
        pooled = F.avg_pool2d(reg_feat, ks, ks)    # [bs, 256, 2, 2]
        cls_pred = self.cf_classifier(pooled)
        
        if isinstance(xin,torch.Tensor):
            xin=[xin]
        
        # predict bounding box for box regression
        ...
        if self.training:
            ...
            return pred.view(batchsize,-1), cls_pred
    ...
    def get_losses(
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            num_gts += num_gt
            if num_gt == 0 or cls_label[batch_idx] == 1:                # or it is counterfactual
